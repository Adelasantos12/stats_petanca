...
export default function LiveMatch() {
  ...
  const fetchMatch = useCallback(async () => {
    try {
      ...
      if (res.data && !selectedPlayerId) {
        const teamPlayers = res.data.players.filter(
          (p: { teamSide: string }) => p.teamSide === selectedTeam,
        );
        if (teamPlayers.length > 0)
          setSelectedPlayerId(teamPlayers[0].playerId);
      }
    } catch (err) {
      console.error(err);
      setLoading(false);
  ...
  const handleUndoLastThrow = async () => {
    if (currentHandThrows.length === 0) return;
    const lastThrow = currentHandThrows.reduce((latest, current) => {
  ...
  const currentHandThrows = match.throws.filter(
    (t) => t.handNumber === currentHandNumber,
  );
  ...
  const ballsPerPlayer =
    match.modality === "SINGLE" ? 3 : match.modality === "DOUBLES" ? 3 : 2;
      ...
      >
        <div className="flex justify-between items-center">
          ...
          <Link
            ...
            className="p-3 bg-indigo-50 text-indigo-600 rounded-2xl transition-all active:scale-90"
          >
            <BarChart2 size={24} />
          </Link>
        </div>
      ...
      {match.status === "FINISHED" ? (
        ...
        >
          ...
          <div className="relative z-10 space-y-6">
            ...
            <div>
              <h3 className="text-4xl font-black text-white tracking-tight uppercase leading-none">
                ¡VICTORIA!
              </h3>
              ...
              <button
                onClick={() => {
                  if (confirm("¿Deseas reabrir la partida?")) {
                    api
                      .post(`/matches/${id}/finish`, { status: "IN_PROGRESS" })
                      .then(() => fetchMatch());
                  }
                }}
        ...
        <>
          ...
          >
            ...
            <div className="space-y-4">
              <div className="flex gap-2">
                <div className="flex-1 flex bg-slate-100/50 rounded-2xl p-1 gap-1">
                  <button
                    ...
                    .map((mp) => {
                      ...
                      return (
                        <motion.button
                          ...
                          className={cn(
                            "flex-shrink-0 px-4 py-3 rounded-xl border-2 transition-all flex flex-col items-center justify-center min-w-[80px]",
                            isSelected
                              ? selectedTeam === "A"
                        ...
                        >
                          <span className="text-[10px] font-black truncate w-16 text-center uppercase tracking-tight mb-1">
                            {mp.player.name}
                          </span>
            ...
            <div className="grid grid-cols-[auto_1fr] gap-4">
              ...
              <div className="flex flex-col gap-2">
                <button
                  ...
                  className={cn(
                    "h-14 w-14 rounded-2xl font-black text-[9px] uppercase tracking-widest border-2 transition-all flex flex-col items-center justify-center gap-1",
                    throwType === "POINT"
                ...
                {[-2, -1, 0, 1, 2].map((score) => {
                  ...
                  return (
                    <motion.button
                      ...
                      className={cn(
                        "h-full rounded-2xl text-xl font-black transition-all flex flex-col items-center justify-center border-2 py-2",
                        isSelected
                          ? isPositive
                            ? "bg-emerald-500 text-white border-emerald-500 shadow-lg shadow-emerald-200 scale-105 z-10"
                            : isNegative
                              ? "bg-rose-500 text-white border-rose-500 shadow-lg shadow-rose-200 scale-105 z-10"
                              : "bg-slate-800 text-white border-slate-800 shadow-lg scale-105 z-10"
                          : "bg-slate-50 text-slate-400 border-transparent",
                      )}
                    >
                      {score > 0 ? `+${score}` : score}
                      <span className="text-[7px] font-black opacity-60 mt-1 uppercase tracking-tighter hidden sm:block">
                        {score === -2
                          ? "Pésimo"
                          : score === -1
                            ? "Mal"
            ...
            {/* Optional inputs row */}
            <div className="flex gap-2">
              <div className="w-24 bg-slate-50/50 rounded-xl px-3 py-2 flex flex-col justify-center border-2 border-transparent focus-within:border-indigo-400 focus-within:bg-white transition-all">
                ...
                <input
                  ...
                  className="w-full bg-transparent border-0 focus:ring-0 outline-none font-bold text-slate-700 text-xs p-0 m-0"
                />
              </div>
              <div className="flex-1 bg-slate-50/50 rounded-xl px-3 py-2 flex flex-col justify-center border-2 border-transparent focus-within:border-indigo-400 focus-within:bg-white transition-all">
                <span className="text-[8px] font-black text-slate-300 uppercase italic">
                  Nota
                </span>
                <input
          ...
          <div className="grid grid-cols-3 gap-4">
            <button
              ...
              disabled={currentHandThrows.length === 0}
              className="glass border-white/50 text-slate-400 p-4 rounded-[2rem] font-black text-[10px] uppercase tracking-widest flex flex-col items-center justify-center gap-2 hover:bg-white hover:text-amber-500 transition-all shadow-sm disabled:opacity-50"
            >
              <RotateCcw size={20} />
              Deshacer Bola
            </button>
            <button
              ...
              className="glass border-white/50 text-slate-400 p-4 rounded-[2rem] font-black text-[10px] uppercase tracking-widest flex flex-col items-center justify-center gap-2 hover:bg-white hover:text-rose-500 transition-all shadow-sm"
            >
              <RotateCcw size={20} />
              Anular Mano
            </button>
            <button
              ...
              className="glass border-white/50 text-slate-400 p-4 rounded-[2rem] font-black text-[10px] uppercase tracking-widest flex flex-col items-center justify-center gap-2 hover:bg-white hover:text-indigo-500 transition-all shadow-sm"
            >
              <AlertCircle size={20} />
              Terminar
            </button>
          ...
          {showHistory && (
            ...
            >
              {[...Array(currentHandNumber)].map((_, i) => {
                ...
                const hThrows = match.throws.filter(
                  (t) => t.handNumber === hNum,
                );
                const handRecord = match.hands.find(
                  (h) => h.handNumber === hNum,
                );
                ...
                return (
                  <div
                    key={hNum}
                    className="glass rounded-[2rem] border-white/50 overflow-hidden shadow-sm"
                  >
                      ...
                      {handRecord ? (
                        handRecord.status === "CANCELED" ? (
                          ...
                          <span className="text-[10px] font-black text-emerald-600 bg-emerald-50 px-4 py-1.5 rounded-full uppercase">
                            {handRecord.pointsTeam === "A"
                              ? match.teamAName
                              : match.teamBName}{" "}
                            +{handRecord.pointsValue}
                          </span>
                        )
                      ) : (
                        <span className="text-[10px] font-black text-indigo-500 bg-indigo-50 px-3 py-1 rounded-full uppercase animate-pulse">
                          En Curso
                        </span>
                      )}
                    </div>
                            ...
                            <div className="flex items-center gap-4 flex-1 overflow-hidden">
                              ...
                              <div className="overflow-hidden">
                                ...
                                <div className="flex items-center gap-2 mt-0.5">
                                  ...
                                  {t.note && (
                                    <span className="text-[9px] text-slate-400 font-medium truncate italic border-l border-slate-200 pl-2">
                                      {t.note}
                                    </span>
                                  )}
                                </div>
            ...
            >
              <div className="text-center">
                <h3 className="text-3xl font-black text-slate-800 uppercase tracking-tight">
                  Cerrar Mano {currentHandNumber}
                </h3>
                <p className="text-slate-400 font-bold mt-2 text-sm">
                  ¿Quién sumó puntos en esta mano?
                </p>
              </div>
              ...
              <div className="space-y-8">
                ...
                <div className="bg-slate-50/50 p-8 rounded-[3rem] space-y-6 border border-slate-100">
                  <span className="block text-center text-[10px] font-black text-slate-300 uppercase tracking-[0.2em]">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {[1, 2, 3, 4, 5, 6].map((v) => (
                      <div key={v} className="flex flex-col gap-2">
                        <button
                          onClick={() => handleCloseHand("A", v)}
                          className="bg-white text-indigo-600 py-3.5 rounded-2xl font-black text-xs shadow-sm active:bg-indigo-50 border border-slate-100"
                        >
                          +A {v}
                        </button>
                        <button
                          onClick={() => handleCloseHand("B", v)}
                          className="bg-white text-rose-600 py-3.5 rounded-2xl font-black text-xs shadow-sm active:bg-rose-50 border border-slate-100"
                        >
                          +B {v}
                        </button>
                      </div>
          ...
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-end sm:items-center justify-center p-4">
            ...
            >
              <div className="text-center">
                ...
                <p className="text-slate-400 font-bold mt-2 text-sm">
                  ¿Deseas concluir la partida ahora?
                </p>
              </div>
